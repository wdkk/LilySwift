...
public func LCImageRawMatrix( _ img_:LCImageSmPtr ) -> LLVoidMatrix? {
    guard let rawimg = img_.rawimg else { return nil }
    return rawimg.getRawMatrix()
}

public func LCImageRGBA8Matrix( _ img_:LCImageSmPtr ) -> LLColor8Matrix? {
    guard let raw_matrix = img_.rawimg?.getRawMatrix() else { return nil }
...
public func LCImageWidth( _ img_:LCImageSmPtr ) -> Int {
    guard let rawimg = img_.rawimg else { return 0 }
...
public func LCImageCheckLoadTypeByExtension( _ option_:LLImageLoadOption, _ ext_:LCStringSmPtr ) -> LLImageLoadType {
    let extc = LCStringToCChars( ext_ )
    // autoではない場合はそのまま返す
...
public func LCImage2CGImage( _ img_:LCImageSmPtr ) -> CGImage? {  
    ...
    guard let nonnull_cg_context = cg_context else { return nil } 
    let cg_img = nonnull_cg_context.makeImage()

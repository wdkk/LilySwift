...
public extension LCImageLoaderInternal
{
    ...
    private func decodeTargaRLE( _ dst:LLUInt8Ptr, _ src:LLUInt8Ptr, _ dst_size:Int, _ src_size:Int, _ mem_size:Int ) {
        ...
        var pos:Int = 0
        var dst_pos:Int = 0
        let decode_size = mem_size / 8 // 1ピクセルのデータサイズ
        var rle_count:Int = 0
        var rle_repeating:Int = 0
        var read_next_pixel:Int = 0

        var dec_ptr = [LLUInt8]( repeating: 0, count: decode_size )
    ...
    func loadTarga( _ file_path_:LCStringSmPtr, _ option_:LLImageLoadOption = LLImageLoadOptionDefault() ) -> LCImageSmPtr {
        ...
        let fr = LCFileReaderMake( file_path_ )
        ...
        // オリジンのチェック
        let x_origin = ( style >> 4 & 0x01 ) == 0 ?  1 : -1
        let y_origin = ( style >> 5 & 0x01 ) == 0 ? -1 :  1
        ...
        if depth == 32 {
            ...
            if image_type == 10 {
                // ヘッダをのぞいたメモリサイズを取得
                let buf_size = LCFileGetSize( file_path_ ).i! - HEADER_SIZE
                let tga_mem_tmp = UnsafeMutablePointer<LLUInt8>.allocate(capacity: buf_size )
            ...
            // カラーマップの取得
            let map_length = color_entry_length.i        // カラーマップの数
            let map_size   = color_entry_depth.i / 8    // バイト数に直す
            var map_color  = UnsafeMutablePointer<LLUInt8>.allocate(capacity: map_length * map_size )
            defer { map_color.deallocate() }
            
            LCFileReaderRead( fr, map_color, (map_length * map_size).i64 )
            
            // tgaメモリ配列の取得
            var tga_mem = UnsafeMutablePointer<LLUInt8>.allocate(capacity: wid * hgt )
            defer { tga_mem.deallocate() }
            
            LCFileReaderRead( fr, tga_mem, (wid * hgt).i64 )
            
            let tga_ptr = tga_mem
            let color_ptr = map_color
            for y in 0 ..< hgt {
                tga_x = tga_xs
                for x in 0 ..< wid {
                    let idx = tga_ptr[ (tga_x + tga_y * wid) ].i

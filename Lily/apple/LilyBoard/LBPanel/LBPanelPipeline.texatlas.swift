...
{
    // アトラスによる複数画像の描画
    static func texatlas( 
        _ label:String = UUID().labelString,
        using atlas:LLMetalTextureAtlas
    )
    -> LBPanelPipeline 
    {
        ...
        .custom( label: lbl )
        .textureAtlas( atlas )  // 注: アトラスの指定が重要
        .vertexShader(
            LLMetalShader(
            code: str_struct +                
            """
            ...
            """,
            shaderName:"LBPanelVertexTexAtlas_\(label)" )
        )
        .fragmentShader(
            LLMetalShader( 
            code: str_struct +
            ...
            fragment
            float4
            LBPanelFragmentTexAtlas_\(label)(
            ...
            """,
            shaderName:"LBPanelFragmentTexAtlas_\(label)" )
        )

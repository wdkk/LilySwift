...
{    
    ...
    {    
        ...
        // バンドル画像からの読み込み
        public init( device:MTLDevice?, named bundle_path:String ) {
            self.device = device
        ...
        // Lily画像オブジェクトからの作成
        public init( device:MTLDevice?, llImage img:LLImage ) {
            self.device = device
        ...
        )
        {
            ...
            self.metalTexture = device?.makeTexture( descriptor: tex_desc )
        }
    }
}

...
{   
    ...
    {   
        ...
        private var actorGroup:[Storage:Set<ModelActor>] = [:]
        
        public func shapes( on storage:Storage? ) -> Set<ModelActor> {
            guard let storage = storage else { return [] }
            return actorGroup[storage] ?? []
        }
        
        public func insert( shape:ModelActor, to storage:Storage? ) {
            guard let storage = storage else { return }
            if actorGroup[storage] == nil { actorGroup[storage] = [] }
            actorGroup[storage]?.insert( shape ) 
        }
        public func remove( shape:ModelActor, to storage:Storage? ) { 
            guard let storage = storage else { return }

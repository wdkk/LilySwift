...
{   
    open class BBShaderString
    {
        ...
        public let playground3DBillboardVertexShader:Lily.Metal.Shader
        public let playground3DBillboardFragmentShader:Lily.Metal.Shader
        ...
        private init( device:MTLDevice ) {
            ...
            self.playground3DBillboardVertexShader = .init(
                device:device, 
                code: Self.importsCode + Self.definesCode + Self.vertexShaderCode,
                shaderName:"Lily_Stage_Playground3D_Billboard_Vs" 
            )
            
            self.playground3DBillboardFragmentShader = .init(
                device:device,
                code: Self.importsCode + Self.definesCode + Self.fragmentShaderCode,
                shaderName:"Lily_Stage_Playground3D_Billboard_Fs" 
            )
        }
    }
}

...
where TColor: LLFloatConvertable & Comparable
{
    ...
    ) 
    {
        ...
        if xInt >= 1 && yInt >= 1 && xInt < srcWidth - 2 && yInt < srcHeight - 2 {
            ...
            // 横方向の補間
            let finalValue = cubicInterpolate(columnResults[0], columnResults[1], columnResults[2], columnResults[3], dy_frac)
            pdst[y][x] = LLWithin(min: TColor.colorRangeMinValue, .init( finalValue ), max:TColor.colorRangeMaxValue ) 
        } 
        else {
